...
(function () {
    IE7 = {
        toString: function () {
            return "IE7 version 2.0 (beta4)"
        }
    };
    var m = IE7.appVersion = navigator.appVersion.match(/MSIE (\d\.\d)/)[1];
    if (/ie7_off/.test(top.location.search) || m < 5) return;
    var U = bT();
    ...
    if (m < 5.5) {
        ...
        Function.prototype.apply = function (a, b) {
            if (a === undefined) a = cG; else if (a == null) a = window; else if (typeof a == "string") a = new String(a); else if (typeof a == "number") a = new Number(a); else if (typeof a == "boolean") a = new Boolean(a);
            if (arguments.length == 1) b = []; else if (b[0] && b[0].writeln) b[0] = b[0].documentElement.document || b[0];
            var c = "#ie7_apply", d;
            a[c] = this;
            switch (b.length) {
                case 0:
    ...
    var H = B.extend({
        constructor: function (a) {
            ...
            if (!this[bz + a]) this[V].push(String(a));
    ...
    function X(a, b, c) {
        ...
        if (!IE7.CSS.print) new bJ("print");
        IE7.CSS.print.recalc()
    });
    var bY = /^\d+(px)?$/i;
    var M = /^\d+%$/;
    var D = function (a, b) {
        if (bY.test(b)) return parseInt(b);
        var c = a.style.left;
    ...
    function cV(a) {
        ...
        IE7.HTML.recalc();
        IE7.CSS.recalc();
        for (var a = 0; a < bE.length; a++) bE[a]()
    };
    ...
    IE7.CSS = new (bZ.extend({
        ...
        recalc: function () {
            ...
            for (r = b; r < c; r++) {
                ...
                if (f && (g = s.match(a))) {
                    j = x(f.selectorText);
                    if (j.length) for (k = 0; k < g.length; k++) {
                        n = g[k];
                        h = IE7.CSS.recalcs[n.slice(10)][2];
                        for (u = 0; (q = j[u]); u++) {
                            if (q.currentStyle[n]) h(q, s)
                        }
                    }
                }
            }
    ...
    var bJ = B.extend({
        constructor: function (a) {
            ...
            function s(a) {
                if (q.test(a)) return "all"; else if (r.test(a)) return (k.test(a)) ? "all" : "screen"; else if (k.test(a)) return "print"
            };var R = this;
    ...
    if (m < 7) {
        ...
        v(document, "onmouseup", function () {
            var a = Z.instances;
            for (var b in a) if (!a[b][0].contains(event.srcElement)) Z.unregister(a[b])
        })
    }
    ...
    new function (_) {
        ...
        function collapseMargin(a, b) {
            if (!a.runtimeStyle[b]) {
                ...
                if (c && IE7.hasLayout(c) && !IE7[b == "marginTop" ? "_4" : "_3"](a)) return;
        ...
        var fixWidth = function (g) {
            applyWidth = function (a) {
                if (!M.test(a.currentStyle.width)) h(a);
                collapseMargins(a)
            };
            ...
            function q(a, b) {
                if (M.test(b)) return parseInt(parseFloat(b) / 100 * j(a));
                return D(a, b)
            };var r = function (a, b) {
                var c = a.currentStyle["box-sizing"] == "border-box";
                var d = 0;
                if (G && !c) d += k(a) + u(a, "padding"); else if (!G && c) d -= k(a) + u(a, "padding");
                return q(a, b) + d
            };
            ...
            function n(a) {
                ...
                } else if (a.currentStyle.maxWidth != "none" && c >= r(a, a.currentStyle.maxWidth)) {
            ...
            bC(function () {
                ...
                for (a in s.elements) R(s.elements[a])
            });
            if (G) {
    ...
    if (m >= 5.5 && m < 7) {
        ...
        IE7.HTML.addRecalc("img,input", function (a) {
            ...
            v(a, "onpropertychange", function () {
                if (!bn && event.propertyName == "src" && a.src.indexOf(bk) == -1) bL(a)
            })
        });
        var bn = false;
        v(window, "onbeforeprint", function () {
            bn = true;
            for (var a = 0; a < Q.length; a++) cp(Q[a])
        });
        v(window, "onafterprint", function () {
            for (var a = 0; a < Q.length; a++) bo(Q[a]);
            bn = false
        })
    }
    ...
    function cp(a) {
        ...
        function _7(a, b) {
            ...
            if (!a.runtimeStyle.autoLeft && a.currentStyle.marginLeft == "auto" && a.currentStyle.right != "auto") {
                ...
                if (_2(a.offsetParent)) a.runtimeStyle.pixelLeft += c; else a.runtimeStyle.shiftLeft = c
            }
            clipWidth(a);
            clipHeight(a)
        };
    ...
    function cq() {
        ...
        if (m < 6) {
            ...
            var q = /em$/i, r = /ex$/i;
            D = function (a, b) {
                if (bY.test(b)) return parseInt(b) || 0;
                var c = h.test(b) ? -1 : 1;
                if (j.test(b)) c *= u(a);
            ...
            function u(a) {
                ...
                while (a != w) {
                    ...
                    if (c) {
                        if (q.test(c)) b *= parseFloat(c); else if (M.test(c)) b *= (parseFloat(c) / 100); else if (r.test(c)) b *= (parseFloat(c) / 2); else {
                            k.style.fontSize = c;
                            return 1
                        }
                    }
        ...
        function n(a) {
            if (m < 5.5) IE7.Layout.boxSizing(a.parentElement);
    ...
    function cr(a, b, c, d) {
        d = /last/i.test(a) ? d + "+1-" : "";
        if (!isNaN(b)) b = "0n+" + b; else if (b == "even") b = "2n"; else if (b == "odd") b = "2n+1";
        b = b.split("n");
    ...
    v(document, "onmouseup", function () {
        var a = bq.instances;
        for (var b in a) bq.unregister(a[b])
    });
    var br = new A("checked", function (a) {
        ...
        IE7.CSS.addEventHandler(a, "onpropertychange", function () {
            if (event.propertyName == "checked") {
                if (a.checked) br.register(b); else br.unregister(b)
            }
        });
        if (a.checked) br.register(b)
    });
    var bs = new A("enabled", function (a) {
        ...
        IE7.CSS.addEventHandler(a, "onpropertychange", function () {
            if (event.propertyName == "disabled") {
                if (!a.isDisabled) bs.register(b); else bs.unregister(b)
            }
        });
        if (!a.isDisabled) bs.register(b)
    });
    var bt = new A("disabled", function (a) {
        ...
        IE7.CSS.addEventHandler(a, "onpropertychange", function () {
            if (event.propertyName == "disabled") {
                if (a.isDisabled) bt.register(b); else bt.unregister(b)
            }
        });
        if (a.isDisabled) bt.register(b)
    });
    var bu = new A("indeterminate", function (a) {
        ...
        IE7.CSS.addEventHandler(a, "onpropertychange", function () {
            if (event.propertyName == "indeterminate") {
                if (a.indeterminate) bu.register(b); else bu.unregister(b)
            }
        });
    ...
    var bv = new A("target", function (a) {
        ...
        IE7.CSS.addEventHandler(document, "onpropertychange", function () {
            if (event.propertyName == "activeElement") {
                if (a.id && a.id == location.hash.slice(1)) bv.register(b); else bv.unregister(b)
            }
        });
        if (a.id && a.id == location.hash.slice(1)) bv.register(b)
    });
    var ct = /^attr/;
    var cu = /^url\s*\(\s*([^)]*)\)$/;
    ...
    var F = IE7.PseudoElement = o.extend({
        constructor: function (a, b, c) {
            ...
            for (var a = 0; a < this.match.length; a++) {
                ...
                if (!b[this.position]) b[this.position] = {cssText: ""};
                b[this.position].cssText += ";" + this.cssText;
                if (this.content != null) b[this.position].content = this.content
            }
        }, create: function (a) {
